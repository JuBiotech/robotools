...
class WellRandomizer:
    """Helper object to rendomize a set of well IDs within a MTP."""
    ...
    def __init__(self, original_shape: tuple, random_seed: int) -> None:
        ...
        self.lookup = {
            owell: rwell
            for owell, rwell in zip(self.original_wells, self.randomized_wells)
        }
        super().__init__()
    ...
    def derandomize_wells(self, wells: ArrayLike) -> numpy.ndarray:
        ...
        """
        self.input_wells = numpy.array(wells)
        self.derandomized_output_wells = [key for key, value in self.lookup.items() if value in self.input_wells]
