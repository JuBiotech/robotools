...
class Worklist(list):
    ...
    def evo_transfer(
        ...
        source_position: typing.Tuple[int, int],
        destination: liquidhandling.Labware,
        destination_wells: typing.Union[str, typing.Sequence[str], numpy.ndarray],
        destination_position: typing.Tuple[int, int],
        volumes: typing.Union[float, typing.Sequence[float], numpy.ndarray],
        tips: typing.Union[typing.List[Tip], typing.List[int]],
        liquid_class: str,
        *,
        ...
        **kwargs,
        ...
        source_position : tuple
            Tuple with position of source labware (grid and site as integers)
        destination : liquidhandling.Labware
            Destination labware
        destination_wells : str or iterable
            List of destination well ids
        destination_position : tuple
            Tuple with position of destination labware (grid and site as integers)
        volumes : float or iterable
            Volume(s) to transfer
        tips: list
            Tips with which to perform the transfer
        liquid_class : str
            Liquid class to use for aspirate and dispense.
        label : str
            Label of the operation to log into labware history
        partition_by : str
            ...
            for p in range(npartitions):
                ...
                for s, d, vs in zip(srcs, dsts, vols):
                    ...
                    if len(vs) > p:
                        v = vs[p]
                        if v > 0:
                            self.evo_aspirate(source, s, source_position, v, liquid_class, tips, label=None)
                            self.evo_dispense(
                                destination, d, destination_position, v, liquid_class, tips, label=None
                            )
                            nsteps += 1
